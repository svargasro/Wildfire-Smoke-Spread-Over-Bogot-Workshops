...
private:
  ...
  vector3D r, rp, F; double theta, m, R, L0;
  vector3D Vaux, Fg;
public:
  vector3D V;
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * moleculas){
  ...
  for(i=0;i<N;i++){
    ...
    m_i = moleculas[i].m;
    y = (moleculas[i].r - moleculas[i].rp).y();
    x = (moleculas[i].r - moleculas[i].rp).x();
    clog<<"x("<<i+1<<")="<<x<<", y("<<i+1<<")="<<y<<endl;
    theta = atan2(y,x);
    clog<<"theta "<<theta<<endl;
    // Fg.load(0,-1*m_i*g,0);
    Fg.load(-1*m_i*g*sin(theta),-1*m_i*g*cos(theta),0);
    clog<<"Fx("<<i+1<<")="<<Fg.x()<<", Fy("<<i+1<<")="<<Fg.y()<<endl;
