...
const double K=4.5e4; //Elasticidad de la colisión
...
public:
  ...
  void FuerzaPerperndicular(Cuerpo & moleculas,double F);
};
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * moleculas){
  ...
  // for(i=0;i<N;i++) {
  //   vector3D Fp;
  //   double x = moleculas[i].r.x();
  //   double y = moleculas[i].r.y();
  //   double v2 = moleculas[i].V.norm2();
  //   double m = moleculas[i].m;
  //   double xp = moleculas[i].rp.x();;
  //   double l = moleculas[i].L0;
  //   clog<<"l="<<l<<endl;
  //   double sinMol = (x-xp)/l;
  //   double cosMol = (l-y)/l;
  //   double T= m*v2/l + g*m*cosMol;
  //   Fp.load(-T*sinMol,-m*g+T*cosMol,0);
  //   moleculas[i].SumeFuerza(Fp);
  // }
  for(i=0;i<N;i++){

    double m = moleculas[i].m;
    double Fg = g*m;
    FuerzaPerperndicular(moleculas[i],Fg);
...
void Colisionador::FuerzaPerperndicular(Cuerpo & moleculas,double F){
    vector3D Fp;
    double x = moleculas.r.x();
    double y = moleculas.r.y();
    double xp = moleculas.rp.x();
    double yp = moleculas.rp.y();
    double v2 = moleculas.V.norm2();
    double m = moleculas.m;
    double l = moleculas.L0;
    // clog<<"l="<<l<<endl;
    double sinMol = (x-xp)/l;
    double cosMol = -(y-yp)/l;
    double T= m*v2/l + F*cosMol;
    Fp.load(-T*sinMol,-F+T*cosMol,0);
    moleculas.SumeFuerza(Fp);
}
...
void Colisionador::CalculeFuerzaEntre(Cuerpo & moleculas1, Cuerpo & moleculas2){
  ...
  if(s>0){
    ...
    FuerzaPerperndicular(moleculas2,aux);
    FuerzaPerperndicular(moleculas1,-1*aux); 
...
void InicieAnimacion(void){
  ...
  cout<<"set output 'PendulosSistemaLab.gif'"<<endl;
  cout<<"unset key"<<endl;//No dibuja la leyenda de los datos
  cout<<"set xrange[-15:"<<Lx + 15<<"]"<<endl;// Rango en x
  cout<<"set yrange[-15:"<<Ly + 15<<"]"<<endl;// Rango en y
...
int main(){
  ...
  int Ncuadros=100; double t,tdibujo,dt=1e-2,tmax=20,tcuadro=tmax/(Ncuadros+1); 
  ...
  double l = 12;
  Vx0=0; Vy0=0;
  theta= M_PI/2 -M_PI/12 ;
  xp=dx; yp=l;
  //Tercer cuadrante
  double x00=-l*cos(theta) + xp , y00=-l*sin(theta) + yp;
  
  moleculas[0].Inicie(x00,y00,xp,yp,0,0,m0,R0);
  // moleculas[0].Inicie(dx,0,dx,l,0,0,m0,R0);
  ...
  for(ix=1;ix<Nx;ix++){
      ...
      moleculas[ix].Inicie(x0,y0,xp,yp,Vx0,Vy0,m0,R0);
  } 
