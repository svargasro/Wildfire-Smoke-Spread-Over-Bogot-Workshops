...
public:
  ...
  void PrintCylinderVelocities(const char * NameFile, int ixc, int iyc, int R, double w);
};  
...
void LatticeBoltzmann::PrintCylinderVelocities(const char * NameFile, int ixc, int iyc, int R, double w) {
  ofstream MyFile(NameFile);
  int ix, iy;
  double Ux0, Uy0;
  double R2 = R * R;

  for(ix = ixc - R; ix <= ixc + R; ix++) {
    for(iy = iyc - R; iy <= iyc + R; iy++) {
      if((ix - ixc) * (ix - ixc) + (iy - iyc) * (iy - iyc) <= R2) {
        Ux0 = -w * (iy - iyc);
        Uy0 = w * (ix - ixc);
        MyFile << ix << " " << iy << " " << Ux0 << " " << Uy0 << endl;
      }
    }
  }
  MyFile.close();
...
//Air.PrintCylinderVelocities("cylinder_velocities.dat", ixc, iyc, R, w);
