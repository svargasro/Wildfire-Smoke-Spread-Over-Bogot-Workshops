...
double LatticeGas::rho(int ix, int iy, bool UseNew)
{
  ...
  if (UseNew)
    for (int i = 0; i < Q; i++)
      sum += n_new[ix][iy][i]; // Revisar scope
  else
    for (int i = 0; i < Q; i++)
      sum += n[ix][iy][i];
  return sum;
  ...
  { // Recorriendo cada celda
    for (iy = 0; iy < Ly; iy++)
    {
      ...
      { // Recorriendo cada dirección
        ...
        if (n_aleatorio < p0)
          n_new[ix][iy][i] = n[ix][iy][i];
        // Se rota la celda 90 grados (derecha)
        else if (n_aleatorio < p0 + p)
          n_new[ix][iy][i] = n[ix][iy][(i + 3) % Q];
        // Se rota la celda 270 grados (izquierda)
        else if (n_aleatorio < p0 + 2 * p)
          n_new[ix][iy][i] = n[ix][iy][(i + 1) % Q];
        // Se rota la celda 180 grados (abajo)
        else
          n_new[ix][iy][i] = n[ix][iy][(i + 2) % Q];
      }
    }
  }
}
...
int main(void)
{
  ...
  for (int i = 0; i < N_automatas; i++)
    Difusion[i].Borrese();
  for (int i = 0; i < N_automatas; i++)
    Difusion[i].Inicie(N_par, mu_xi, mu_yi, sigma_xi, sigma_yi, ran64);
  ...
  for (int t = 0; t < tmax; t++)
  {
    ...
    for (int i = 0; i < N_par; i++)
      DibujeCuadro(mu_xi, mu_yi, Lx);
    TermineCuadro();

    for (int i = 0; i < N_automatas; i++)
      Difusion[i].Colisione(ran64);
    for (int i = 0; i < N_automatas; i++)
      Difusion[i].Adveccione();
  }
  TermineAnimacion();
  return 0;
}