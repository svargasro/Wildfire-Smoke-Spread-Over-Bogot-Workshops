...
utils.readParamsFromTable(
        descriptionIn='noDescription',
        frIn=0.5,
        enIn=0.01,
        etIn=0.01,
        tcIn=0.0001,  # Frictionangle (rad), strength-parameter
...
mat1 = O.materials.append(ViscElMat(
        frictionAngle=frIn,
        density=rhoIn,
        tc=tcIn,
        en=enIn,
...
o.engines = [
        ...
        CircularFactory(
                maxParticles=1500,  #Number of generated particles
        ...
        RotationEngine(
                ids=id_HourGl,
                angularVelocity=math.pi / rotPeriod,
        ...
        VTKRecorder(
                iterPeriod=dumpVTKIn,
                fileName=folderName + '/spheres-',
                mask=2,
                recorders=['spheres', 'velocity', 'colors', 'intr', 'ids', 'mask', 'materialId', 'stress'],
                label='VTK_box1'
        ),
        VTKRecorder(iterPeriod=dumpVTKIn, fileName=folderName + '/facetB-', mask=4, recorders=['facets'], label='VTK_box2'),
