...
void DibujarPendientes(Cuerpo granoTop, Cuerpo granoLeft, Cuerpo granoRight){ 
  //Dibuja las pendientes calculadas en la función AnguloReposo
  cout << " , " << granoLeft.Getx() <<"+"<<(granoTop.Getx()-granoLeft.Getx())/7 << "*t,"; 
  cout << granoLeft.Gety() <<"+"<<(granoTop.Gety()-granoLeft.Gety())/7 << "*t"; //Dibuja la pendiente1
  cout << " , " << granoTop.Getx() <<"+"<<(granoRight.Getx()-granoTop.Getx())/7 << "*t,"; 
  cout<< granoTop.Gety() <<"+"<<(granoRight.Gety()-granoTop.Gety())/7 << "*t"; //Dibuja la pendiente2
}
...
double AnguloReposo(Cuerpo * Grano){
  //Variables (índices) de los granos extremos para calcular las pendientes
  int granoYMax=0, granoXIzquierda=0, granoXDerecha=0; 
  for(int i=1; i<N; i++){
