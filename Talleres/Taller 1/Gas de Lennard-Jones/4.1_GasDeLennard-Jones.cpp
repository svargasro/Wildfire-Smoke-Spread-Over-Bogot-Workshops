...
void Colisionador::CalculeFuerzaEntre(Cuerpo & moleculas1, Cuerpo & moleculas2){
  ...
  //Sumar las fuerzas
  moleculas1.SumeFuerza(-1*F1);  
  moleculas2.SumeFuerza(F1*(1));
...
int main(){
  ...
  int Ncuadros=1e3; double t,tdibujo,dt=1e-3,tmax=1e4/*Lx/V0*/,tcuadro=tmax/Ncuadros; 
  ...
  //Particula fuente
  moleculas[N].Inicie(Lx/2,Ly/2, 0,  0, 0,  0,1e0*m0,2*R0); //Pared arriba
  ...
  for(t=tdibujo=0;t<tmax;t+=dt,tdibujo+=dt){
    ...
    if(tdibujo>=tcuadro){
      ...
      InicieCuadro();
      for(i=0;i<N+1;i++) moleculas[i].Dibujese();
      TermineCuadro();
